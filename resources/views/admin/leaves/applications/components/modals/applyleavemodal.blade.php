    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form method="POST" action="{{ route('admin.apply.leave.employee') }}" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                    <div class="form-group">
                        <label>Employee <span class="text-danger">*</span></label>
                        <select class="select" name="add_leave_employee" id="add_leave_employee" onchange="employeeChangeFunc();">
                            <option value="">Select</option>
                            @foreach ($employees as $employee)
                            <option value="{{ $employee->id }}" @if (old('add_leave_employee')==$employee->id) selected @endif>
                                {{ $employee->first_name }} {{ $employee->last_name }}
                            </option>
                            @endforeach
                        </select>
                        <x-field-validation errorname="add_leave_employee" />
                    </div>
                    <div class="form-group">
                        <label>Subject <span class="text-danger">*</span></label>
                        <input class="form-control" type="text" name="add_leave_subject" value="{{ old('add_leave_subject') }}" placeholder="Please enter your subject">
                    ...
                    <div class="form-group">
                        ...
                        <div class="cal-icon">
                            <input class="form-control datetimepicker" type="text" name="add_leave_from" id="add_leave_from" value="{{ old('add_leave_from') }}">
                    ...
                    <div class="form-group">
                        ...
                        <div class="cal-icon">
                            <input class="form-control datetimepicker" type="text" name="add_leave_to" id="add_leave_to" value="{{ old('add_leave_to') }}">
                            <x-field-validation errorname="add_leave_to" />
                        </div>
                    </div>
                    <button class="btn btn-primary btn-sm" type="button" onclick="adjustleavedates(event);">Adjust</button>
                    <div id="adjustdates">
                        @if ($errors->any())
                        @if (count(old('add_leave_adjust_date', [''])) > 0)
                        @for ($i = 0; $i < count(old('add_leave_adjust_date', [''])); $i++) <div class="row">
                            <div class="col-lg-6">
                                <div class="form-group">
                                    <label>Date <span class="text-danger">*</span></label>
                                    <div class="cal-icon">
                                        <input class="form-control" type="text" name="add_leave_adjust_date[]" id="add_leave_adjust_date" value="{{ old('add_leave_adjust_date.' . $i) }}" readOnly>
                            ...
                            <div class="col-lg-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="add_leave_adjust_plan[]" onchange="adjustLeaveHF(event, {{ $i }})">
                                        <option value="">--Select--</option>
                            ...
                            <div class="col-lg-12" id="hour_field_{{ $i }}" style="display: none;">
                                <div class="form-group">
                                    <label>Hours <span class="text-danger">*</span></label>

                                    <input class="form-control" type="number" name="add_leave_adjust_hour[]" id="add_leave_adjust_hour" value="{{ old('add_leave_adjust_hour.' . $i) ?? 0 }}" step="1">

                                </div>
                            </div>
                            <div class="col-lg-12" id="half_day_field_{{ $i }}" style="display: none;">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="add_leave_half_day[]" value="true" id="half_day_leave">
...
<script>
    ...
    $(window).on('load', function() {
        ...
        let selectValue = $('#add_leave_employee').val();
        $.ajax({
            url: `{{ route('admin.leave.application.get') }}?employee_id=${selectValue}`, // Replace with your API endpoint
            method: 'GET',
            dataType: 'json',
            success: function(data) {
                // Handle the successful response
                console.log('Data Main:', data);
                leavePlans = [...data.leave_plans];
    ...
    function parseDate(dateString) {
        ...
        var created_date = new Date(parts[2], parts[0] - 1, parts[1]);
        console.log('date created : ' + created_date);
        return created_date;
    }
    ...
    function employeeChangeFunc() {
        ...
        // AJAX request using $.ajax() with CSRF token in headers
        $.ajax({
            url: `{{ route('admin.leave.application.get') }}?employee_id=${selectValue}`, // Replace with your API endpoint
            method: 'GET',
            dataType: 'json',
