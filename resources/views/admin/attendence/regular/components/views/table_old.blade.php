...
@include('admin.attendence.regular.components.modals.edit-device-log')
@push('modal-script')
    ...
    <script>
        ...
        function openLogModal(attendence) {
            ...
            console.log("test attenance", attendence);
            let mainString = `
            <table class="table">
                <thead>
                    <tr>
            ...
            for (let index = 0; index < attendence['logs'].length; index++) {
                ...
                <tr>
                    ...
                    <td>${remarks}</td>
                    <td><button class="btn btn-primary" onclick="editLog(${element['id']})">Edit</button></td>
                    <td><button class="btn btn-danger" onclick="deleteLog(${element['id']})">Delete</button></td>
                </tr>
                `;
            }
        ...
        function editLog(logId) {
            fetch(`/admin/device-logs/${logId}/edit`) 
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        document.getElementById('device_id').value = data.device_log.device_id;
                        document.getElementById('time').value = data.device_log.time;
                        document.getElementById('type').value = data.device_log.type;
                        document.getElementById('imported').value = data.device_log.imported;
                        document.getElementById('date').value = data.device_log.date;
                        document.getElementById('log_id').value = data.device_log.id;

                        // Open the modal
                        new bootstrap.Modal(document.getElementById('editDeviceLogModal')).show();
                    }
                })
                .catch(() => Swal.fire('Error!', 'An error occurred while fetching data.', 'error'));
        }

        document.getElementById('editDeviceLogForm').addEventListener('submit', function (e) {
            e.preventDefault();

            const formData = new FormData(this);
            
            fetch(`/admin/device-logs/${formData.get('id')}`, {
                method: 'POST',
                headers: {
        ...
        function deleteLog(logId) {
            Swal.fire({
                title: 'Are you sure?',
                text: "This will permanently delete the device log.",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'Yes, delete it!',
            }).then(result => {
                if (result.isConfirmed) {
                    fetch(`/admin/device-logs/${logId}`, {
                        method: 'DELETE',
